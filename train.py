...
parser.add_argument(
    "--dataset", default="cifar10", type=str, help="dataset: [cifar10|stl10|svhn]"
)
parser.add_argument(
    "--model", default="resnet", type=str, help="resnet is supported currently"
)
parser.add_argument(
    "--layers",
    default=0,
...
parser.add_argument(
    ...
    default=0.0,
    type=float,
)  # \lambda_1 for 1st local module
parser.add_argument(
    "--ixy_1",
    default=1.0,
    type=float,
)  # \lambda_2 for 1st local module
parser.add_argument(
    "--ixx_2",
    default=0.0,
    type=float,
)  # \lambda_1 for (K-1)th local module
parser.add_argument(
    "--ixy_2",
    default=1.0,
    type=float,
)  # \lambda_2 for (K-1)th local module
...
parser.add_argument(
    "--rule", default="AugLocal", type=str, help="DGL, InfoPro, PredSim, AugLocal"
)
parser.add_argument(
    "--save_path",
    default="",
...
parser.add_argument(
    ...
    default=0,
    type=int,
    help="predefined depth of auxiliary networks in AugLocal",
)
...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="enable pyramidal depth in AugLocal",
)
parser.add_argument(
    "--pyramid_coeff", default=0.5, type=float, help="coefficient of pyramidal depth"
)
...
training_configurations = {
    "resnet": {
        ...
        "initial_learning_rate": (
            args.lr if args.dataset in ["cifar10", "svhn"] else 0.1
        ),
        "changing_lr": [80, 120],
        "lr_decay_rate": 0.1,
        "momentum": 0.9,
...
def main():
    ...
    if "cifar" in args.dataset:
        ...
        transform_train = transforms.Compose(
            [transforms.RandomCrop(32, padding=2), transforms.ToTensor(), normalize]
        )
        image_size = 32
    else:
    ...
    for epoch in range(start_epoch, training_configurations[args.model]["epochs"]):
        ...
        # train for one epoch
        train_acc, loss_output, loss_x, loss_y = train(
            train_loader, model, optimizer, epoch, None
        )

        # evaluate on validation set
        prec1 = validate(val_loader, model, epoch, None, class_num)
...
def train(train_loader, model, optimizer, epoch, writer):
    ...
    losses_ixx = [
        AverageMeter() for _ in range(args.local_module_num - 1)
    ]  # additional loss term in PredSim and InfoPro
    ...
    for i, (x, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output, loss, _, loss_ixx_list, loss_ixy_list = model(
            img=x,
            target=target,
            ixx_1=args.ixx_1,
            ixy_1=args.ixy_1,
        ...
        if (i + 1) == len(train_loader):
            ...
            string += "mem={:.0f}MiB, max_mem={:.0f}MiB, max_mem={:.0f}MiB \n".format(
...
def validate(val_loader, model, epoch, writer, class_num):
    """Perform validation on the validation set"""
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1_hidden = [AverageMeter() for _ in range(args.local_module_num)]
    top1_hidden_per_class = [
        [AverageMeter() for _ in range(class_num)] for _ in range(args.local_module_num)
    ]
    top1 = AverageMeter()
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        with torch.no_grad():
            output, loss, hidden_logits, _, _ = model(
                img=input, target=target, record_logits=True
            )

        module_i = 0
        for hidden_logits in hidden_logits:
            acc_hidden, acc_hidden_per_class, count_per_class = accuracy_per_class(
                hidden_logits.data, target, topk=(1,)
            )
            top1_hidden[module_i].update(acc_hidden[0].item(), hidden_logits.size(0))
            for n, (class_acc, class_count) in enumerate(
                zip(acc_hidden_per_class, count_per_class)
            ):
                top1_hidden_per_class[module_i][n].update(class_acc.item(), class_count)
            module_i += 1
    ...
    string = (
        ...
        "Time {batch_time.value:.3f} ({batch_time.ave:.3f})\t"
        "Loss {loss.value:.4f} ({loss.ave:.4f})\t"
        "Prec@1 {top1.value:.3f} ({top1.ave:.3f}) (var: {top1.variance:.3f})\n".format(
            epoch,
            (i + 1),
            train_batches_num,
            batch_time=batch_time,
    ...
    for module_num in range(args.local_module_num):
        string += "layer [{0}], Prec@1 {top1.value:.3f} ({top1.ave:.3f}) (var: {top1.variance:.3f})\n".format(
            module_num, top1=top1_hidden[module_num]
        )
    string += "\n"
    for num_class in range(class_num):
        string += f"\nClass {num_class}:\n"
        for module_num in range(args.local_module_num):
            string += "layer [{0}], Prec@1 {top1.value:.3f} ({top1.ave:.3f}) (var: {top1.variance:.3f})\n".format(
                module_num, top1=top1_hidden_per_class[module_num][num_class]
            )
    # writer.add_scalar(f"layer [{module_num}]", top1_hidden[module_num].ave, epoch)
    logging.info(string)
...
class AverageMeter(object):
    ...
    def update(self, val, n=1):
        ...
        if self.count > 1:
            ...
            square_of_mean = self.ave * self.ave
            self.variance = mean_of_squares - square_of_mean
        else:
            self.variance = 0
    ...
    if not args.cos_lr:
        if epoch in training_configurations[args.model]["changing_lr"]:
            for param_group in optimizer.param_groups:
                param_group["lr"] *= training_configurations[args.model][
                    "lr_decay_rate"
                ]
        print("lr:")
        for param_group in optimizer.param_groups:
        ...
        for param_group in optimizer.param_groups:
            if epoch <= 10:
                param_group["lr"] = (
                    0.5
                    * training_configurations[args.model]["initial_learning_rate"]
                    * (
                        1
                        + math.cos(
                            math.pi
...
def accuracy_per_class(output, target, topk=(1,)):
    """Computes the precision@k for the specified values of k and per-class accuracy"""
    maxk = max(topk)
    batch_size = target.size(0)
    ...
    # Initialize tensors for per-class accuracy calculation
    class_correct = torch.zeros(target.max().item() + 1)
    class_total = torch.zeros_like(class_correct)

    # Calculate per-class correct counts and totals
    for i in range(batch_size):
        label = target[i].item()
        class_total[label] += 1
        # Only consider the top-1 prediction for per-class accuracy
        if correct[0, i]:
    ...
    # Handle cases where the class is not present in the batch
    class_accuracy[class_total == 0] = torch.tensor(float("nan"))

    # Combine overall and per-class accuracies
    return res, class_accuracy, class_total
