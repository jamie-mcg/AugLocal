...
class LocalResNet(nn.Module):
    ...
    def __init__(
        ...
        assert arch in ["resnet32", "resnet110"], (
            "This repo supports resnet32 and resnet110 currently. "
        ...
        self.conv1 = nn.Conv2d(
            3, wide_list[0], kernel_size=3, stride=1, padding=1, bias=False
        )
        ...
        if self.is_pyramid:
            self.aux_depth_list = [
                max(
                    ...
                    round(
                        aux_net_depth
                        * (1 - (i * paramid_coeff) / (self.local_module_num - 2))
                        + (i * paramid_coeff) / (self.local_module_num - 2) * 1
                    ),
                )
        ...
        if local_module_num == 1:
            ...
            if rule == "AugLocal":
                self.net_config = network_config[arch]["net_config"][
                    : local_module_num - 1
                ]
            else:
        ...
        if rule == "InfoPro":
            if "cifar" in dataset:
                self.mask_train_mean = (
                    torch.Tensor([x / 255.0 for x in [125.3, 123.0, 113.9]])
                    .view(1, 3, 1, 1)
                    .expand(batch_size, 3, image_size, image_size)
                    .cuda()
                )
                self.mask_train_std = (
                    torch.Tensor([x / 255.0 for x in [63.0, 62.1, 66.7]])
                    .view(1, 3, 1, 1)
                    .expand(batch_size, 3, image_size, image_size)
                    .cuda()
                )
                ...
                self.mask_train_mean = (
                    torch.Tensor([x / 255.0 for x in [127.5, 127.5, 127.5]])
                    .view(1, 3, 1, 1)
                    .expand(batch_size, 3, image_size, image_size)
                    .cuda()
                )
    ...
    def _image_restore(self, normalized_image):
        return (
            normalized_image.mul(self.mask_train_std[: normalized_image.size(0)])
    ...
    def _local_forward(self, x, img, target, stage_i, layer_i):
        if self.training:
            if self.rule == "InfoPro":
                loss_ixx = eval("self.decoder_" + str(stage_i) + "_" + str(layer_i))(
                    x, self._image_restore(img)
                )
                ...
                loss_ixx = torch.tensor(0.0, device=x.device)
            loss_ixy, hidden_logits = eval(
                "self.aux_classifier_" + str(stage_i) + "_" + str(layer_i)
            )(x, target)

        else:
            loss_ixy, hidden_logits = eval(
                "self.aux_classifier_" + str(stage_i) + "_" + str(layer_i)
            )(x, target, log_classes=True)
            loss_ixx = torch.tensor(0.0, device=x.device)
        return (
            hidden_logits,
            loss_ixy,
    ...
    def forward(
        ...
        for stage_i in (0, 1, 2, 3):
            for layer_i in range(self.layers[stage_i]):
                ...
                if local_module_i <= self.local_module_num - 2:
                    if (
                        self.net_config[local_module_i][0] == stage_i
                        and self.net_config[local_module_i][1] == layer_i
                    ):
                        hidden_logits, loss_ixy, loss_ixx = self._local_forward(
                            x, img, target, stage_i, layer_i
                        )
                        if record_logits:
                            hidden_logits_record.append(hidden_logits)
                        if self.training:
                            ratio = (
                                local_module_i / (self.local_module_num - 2)
                                if self.local_module_num > 2
                                else 0
                            )
        ...
        if record_logits:
            hidden_logits_record.append(logits)
